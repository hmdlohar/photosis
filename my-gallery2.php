...
<?php
	if($user[0]['id']== $_SESSION['userLogged'] ){
		echo '<a class="btn btn-primary" href="profile.php"style=";color:black;width:250px;height:40px;margin-top:10px;margin-left:20px;font-weight:bold;font-size:20px; background-color:white; border:3px solid black;"  class="breadcrumb">
						Edit Profile
					</a>';
	}
	else  {
		$flw=sql_query("select * from follow where user_id={$_GET['clicked-id']}");
		$flag=0;
		foreach ($flw as $key => $value)
		{
			if($value['follower_id']==$_SESSION['userLogged']){
				$flag=1;
			}
		}
		if($flag==0){
		echo '<a class="btn btn-primary" id="follow" style="width:250px;height:40px;margin-top:10px;margin-left:20px;font-weight:bold;font-size:20px; background-color:#1380de;"  onclick="fnDoFollow(this,'.$_GET['clicked-id'].')" class="breadcrumb">
						FOLLOW
					</a> ';}
		else{
		echo '<a class="btn btn-primary" id="following" style="width:250px;height:40px;margin-top:10px;margin-left:20px;font-weight:bold;font-size:20px; background-color:#a2c9d8;border-color: #1380de;" onclick="fnDoUnfollow(this,'.$_GET['clicked-id'].')" class="breadcrumb">
						FOLLOWING
					</a>';}
	}
?>			
					<div class="row" style="margin-top:10px">
						...
						<div class="col-lg-4" >
							<span style="font-weight:600; color:blue;" id="followers">
								<?php
						...
						<div class="col-lg-4" >
							<span style="font-weight:600; color:blue;">
								<?php
									$total_following=sql_query("SELECT count(*) as pid from follow where follower_id ={$_GET['clicked-id']}");
									foreach ($total_following as $key => $value)
									{
										echo $value['pid'].' ';
    ...
    <div class="row">
		<div class="col-lg-8">
			<h1 class="" style="font-weight:bold; color:#2d2756;">Gallery</h1>
		</div>
		<div class="col-lg-2 	" >	
				<h3 style="cursor:pointer;" class="active"onclick="window.location='my-gallery2.php?clicked_id={$_GET['clicked-id']}';"><i class="fa fa-book"></i><?php echo " "?>Gallery</h3>
		</div>
		<div class="col-lg-2 ">	
				<a href="love2.php?clicked-id=<?php echo $_GET['clicked-id'] ?> "><h3 style="cursor:pointer;"><i class="fa fa-heart"></i><?php echo " "?>Loves</h3></a>
		</div>
	...
	<ol class="breadcrumb"style="background-color:#404042">
		...
		<li class="breadcrumb-item active"style="color:white">Gallery</li>
	</ol>
	<!---gallery making-->
	<div class="row">
      	<?php 
			...
			foreach ($userPhotos as $key => $value)
			{
				...
				echo '<div class="gallery col-lg-4 col-sm-6 portfolio-item">
			          <div class="card h-100 rounded">
		                <a href="my-gallery2.php?clicked-id='.$value['uid'].'" class="text-info" style="text-align:center;id="username" text-decoration:none;">'.$value['username'].'</a>
		...
		$.ajax({
			...
			success: function(data){
				if(data == "success")
				{
					...
					$(element).attr("onclick","fnDoUnfollow(this, "+user_id+")");
					$("#followers").html(parseInt($("#followers").html()) + 1);
					notie.alert({text: "Following...",type:1});
				}
				else{
					notie.alert({text: "Something went Wrong with Following. ",type:3});
		...
		$.ajax({
			...
			success: function(data){
				if(data == "success")
				{
					...
					$(element).html("FOLLOW");
					$(element).attr("onclick","fnDoFollow(this, "+user_id+")");
					$("#followers").html(parseInt($("#followers").html()) - 1);
					notie.alert({text: "Not Following. ",type:2});
				}
				else{
					notie.alert({text: "Something went Wrong with Following. ",type:3});
