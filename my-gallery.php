...
<?php
	if($user[0]['id']== $_SESSION['userLogged'] ){
		...
		echo ' <a class="btn btn-primary" id="follow" style="width:250px;height:40px;margin-top:10px;margin-left:20px;font-weight:bold;font-size:20px; background-color:#1380de;"  class="breadcrumb">
						FOLLOW
					</a> 
					<a class="btn btn-primary"id="following" style="display:none;width:250px;height:40px;margin-top:10px;margin-left:20px;font-weight:bold;font-size:20px; background-color:#white;"  class="breadcrumb">
						FOLLOWING
					</a>';
	}
?>			
					<div class="row" style="margin-top:10px">
						...
						<div class="col-lg-4" >
							<span style="font-weight:600; color:blue;">
								<?php
									$total_follower=sql_query("SELECT count(*) as pid from follow where user_id = {$_SESSION['userLogged']}");
						...
						<div class="col-lg-4" >
							<span style="font-weight:600; color:blue;">
								<?php
									$total_following=sql_query("SELECT count(*) as pid from follow where follower_id = {$_SESSION['userLogged']}");
    ...
    <div class="row">
		...
		<div class="col-lg-2 	" >	
				<h3 style="cursor:pointer;" class="active"onclick="window.location='my-gallery.php';"><i class="fa fa-book"></i><?php echo " "?>Gallery</h3>
		</div>
		<div class="col-lg-2 ">	
			...
			foreach ($userPhotos as $key => $value)
			{
				...
				echo '<div class="gallery col-lg-4 col-sm-6 portfolio-item">
			          <div class="card h-100 rounded">
			            ...
			            <div class="card-body">
			              <h4 class="card-title">
			                <a class="text-left " style="text-decoration:none;text-align:center;font-weight:bold; color:#1bc70ff5;"href="ieditor.php?user_photo='.$value['pid'].'">'.$value['pname'].'</a>
			                <div class="btn btn-danger float-right" style="background-color:#ce1414;"onclick="deleteUserPhoto('.$value['pid'].',this)"><i class="fa fa-trash"></i><small class="d-none d-sm-block"></small></div>
			                <div class="row"  style="margin-top:10px">';
			                if($value['isLiked'] == 1){
			                	echo '
								<div class="col-md-9">
									<div ><i class="fa fa-heart" style="transition: color 0.5s;color:red;" onclick="fnDoUnlike(this,'.$value['pid'].')"> </i>
										<span id="count">'.$count[0]['like_count'].'</span>
									</div>
								</div>';
			                }
			                else{
