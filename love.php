...
$userPhotos=sql_query("SELECT  user_photos.description as description,users.id as uid,  users.name as uname, user_photos.name as pname,user_photos.id as pid,username, (case when EXISTS(select * from likes where user_id={$_SESSION['userLogged']} and photo_id=user_photos.id) then 1 else 0 end) as isLiked from user_photos,users where user_photos.user_id = users.id and user_photos.id in (Select photo_id from likes where user_id = {$_SESSION['userLogged']}) order by user_photos.time desc");

//$userPhotos=sql_query("SELECT users.name as uname, user_photos.name as pname,user_photos.id as pid,username where user_photos.user_id = users.id and user_photos.id in (Select photo_id from likes where user_id = {$_SESSION['userLogged']}) ");
